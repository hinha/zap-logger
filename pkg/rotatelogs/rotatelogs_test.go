...
func TestLogRotate(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		t.Run(tc.Name, func(t *testing.T) {
			dir, err := ioutil.TempDir("", fmt.Sprintf("rotatelogs-test%d", i))
...
func TestLogRotationCount(t *testing.T) {
	dir, err := ioutil.TempDir("", "rotatelogs-rotationcount-test")
	if !assert.NoError(t, err, "creating temporary directory should succeed") {
		return
	}
	defer os.RemoveAll(dir)
...
func TestLogSetOutput(t *testing.T) {
	dir, err := ioutil.TempDir("", "rotatelogs-test")
	if err != nil {
...
func TestGHIssue16(t *testing.T) {
	...
	dir, err := ioutil.TempDir("", "rotatelogs-gh16")
	if !assert.NoError(t, err, `creating temporary directory should succeed`) {
		return
	}
	defer os.RemoveAll(dir)
...
func TestRotationGenerationalNames(t *testing.T) {
	dir, err := ioutil.TempDir("", "rotatelogs-generational")
	if !assert.NoError(t, err, `creating temporary directory should succeed`) {
		return
	}
	defer os.RemoveAll(dir)
...
func TestForceNewFile(t *testing.T) {
	dir, err := ioutil.TempDir("", "rotatelogs-force-new-file")
	if !assert.NoError(t, err, `creating temporary directory should succeed`) {
		return
	}
	defer os.RemoveAll(dir)
